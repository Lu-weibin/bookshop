...
public class CartController {
    ...
    public Result save(@PathVariable int bookId) {
        ...
        Book book = bookService.findById(bookId).orElse(new Book());
        if (book.getUser().getId().equals(userId)) {
            return new Result(false, "不要加入自己的图书！");
        }
        // 校验该用户是否已加入过该书籍
        Cart existCart = cartService.findByUserIdAndBookIdAndState(userId, bookId, 1);
        if (existCart != null) {
            return new Result(false, "已加入,不要重复添加！");
        }
        Cart cart = new Cart();
        ...
        // 1为加入的状态；2为结算
        cart.setState(1);
        cart.setCreateTime(CommonUtil.now());
